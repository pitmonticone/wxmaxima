...
void EditorCell::Recalculate(AFontSize fontsize) {
  ...
  for (auto &textSnippet : m_styledText) {
    if ((textSnippet.GetText().StartsWith(wxS('\n')) ||
      ...
      m_configuration->GetDC()->GetTextExtent(textSnippet.GetText(), &tokenwidth, &tokenheight);
